...
data "aws_iam_role" "cluster_autoscaler_role" {
  name = "${var.eks_cluster_id}-autoscaler"
}
...
resource "kubernetes_manifest" "cluster_autoscaler_role_1" {
  manifest = yamldecode(<<-EOF
...
resource "kubernetes_manifest" "cluster_autoscaler_role_2" {
  manifest = yamldecode(<<-EOF
    apiVersion: rbac.authorization.k8s.io/v1
    kind: Role
    metadata:
...
resource "kubernetes_manifest" "cluster_autoscaler_role_binding_service_account" {
  manifest = yamldecode(<<-EOF
    apiVersion: rbac.authorization.k8s.io/v1
...
resource "kubernetes_manifest" "autoscaler_role_binding_service_account" {
  manifest = yamldecode(<<-EOF
    apiVersion: rbac.authorization.k8s.io/v1
    kind: RoleBinding
    metadata:
